...
### 缺点
外观模式的缺点
* 不能很好地显示客户使用子系统类，如果对客户访问子系统类做太多的限制则减少了可变性和灵活性。
...
不要通过继承一个外观类在子系统中加入新的行为，这种做法是错误的，外观模式的用意是为子系统提供一个集中化和简化的沟通渠道，而不是向子系统中加入新的行为，新的行为的增加应该通过修改原有子系统类或增加新的子系统类来实现，不能通过外观类来实现。
